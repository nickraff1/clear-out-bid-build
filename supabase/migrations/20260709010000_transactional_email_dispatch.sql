...
FOR EACH ROW
EXECUTE FUNCTION public.notifications_set_email_flag();
  ...
  IF NEW.sender_id = c.buyer_id THEN
    PERFORM public.notify_org(c.seller_org_id, 'new_message', 'New message from buyer',
      CASE
        ...
        ELSE COALESCE('Re: '||lot_title, 'You have a new message')
      END,
...
BEGIN
  ...
  SELECT count(*) INTO previous_bid_count
    FROM public.bids
...
CREATE OR REPLACE FUNCTION public.notify_on_lot_published()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public
...
FOR EACH ROW
EXECUTE FUNCTION public.notify_on_lot_published();
...
BEGIN
  WITH watched_users AS (
    ...
    FROM public.lots l
    JOIN public.watchlist w ON w.lot_id = l.id
    WHERE l.pricing_type = 'auction'
      AND l.status = 'active'
      AND l.auction_end > now()
      AND l.auction_end <= now() + interval '24 hours'
  ),
  bidder_users AS (
  ...
  targets AS (
    ...
    UNION
    SELECT * FROM bidder_users
  ),
  inserted AS (
    INSERT INTO public.notifications (
      user_id, type, title, message, related_lot_id, link_url,
      email_should_send, priority, data
    )
    SELECT
      t.user_id,
  ...
  WITH target_orders AS (
    ...
    JOIN public.clearance_events ce ON ce.id = l.event_id
    WHERE o.agreed_pickup_at > now()
      AND o.agreed_pickup_at <= now() + interval '24 hours'
      AND o.status IN ('paid', 'ready_for_pickup')
  ),
  buyer_inserted AS (
    INSERT INTO public.notifications (
      user_id, type, title, message, related_order_id, related_lot_id, link_url,
      email_should_send, priority, data
    )
    SELECT
      o.buyer_id,
    ...
    WHERE NOT EXISTS (
      ...
      WHERE n.user_id = o.buyer_id
        AND n.type = 'pickup_reminder'
        AND n.related_order_id = o.id
    )
    RETURNING 1
  ),
  seller_targets AS (
    ...
    FROM target_orders o
    JOIN public.org_members om ON om.org_id = o.seller_org_id
  ),
  seller_inserted AS (
    INSERT INTO public.notifications (
      user_id, type, title, message, related_order_id, related_lot_id, link_url,
    ...
    WHERE NOT EXISTS (
      ...
      WHERE n.user_id = s.user_id
        AND n.type = 'pickup_reminder'
        AND n.related_order_id = s.id
    )
    RETURNING 1
  )
  SELECT
    (SELECT count(*) FROM buyer_inserted) + (SELECT count(*) FROM seller_inserted)
    INTO pickup_count;

  RETURN jsonb_build_object(
    'auction_ending_soon_notifications', auction_count,
    'pickup_reminder_notifications', pickup_count
  );
END;
$$;
...
BEGIN
  ...
  IF TG_OP='UPDATE' AND NEW.manual_payout_status IS DISTINCT FROM OLD.manual_payout_status THEN
    IF NEW.manual_payout_status='manual_payout_paid' THEN
      PERFORM public.notify_org(seller_org, 'payout_paid', 'Payout paid',
        format(
          'Your payout for "%s" has been marked paid.%s',
          COALESCE(lot_title,'(lot)'),
          CASE
            WHEN NEW.manual_payout_reference IS NOT NULL THEN ' Reference: ' || NEW.manual_payout_reference || '.'
            ELSE ''
          END
        ),
  ...
  IF TG_OP='INSERT' AND NEW.status='succeeded' AND NEW.manual_payout_status='manual_payout_pending' THEN
    ...
    PERFORM public.notify_admins('admin_payout_pending', 'Manual payout pending',
      'A new paid order needs payout review.', '/app/admin/payouts',
