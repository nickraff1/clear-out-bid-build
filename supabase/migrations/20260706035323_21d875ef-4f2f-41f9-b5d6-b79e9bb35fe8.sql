...
BEGIN
  ...
  IF v_lot.pricing_type <> 'auction' THEN
    RAISE EXCEPTION 'Only auction lots can be relisted';
  END IF;
  ...
  INSERT INTO public.lot_media (lot_id, url, position, alt_text, media_type, thumbnail_url)
  SELECT v_new_id, url, position, alt_text, media_type, thumbnail_url
...
GRANT EXECUTE ON FUNCTION public.relist_auction_lot(uuid, timestamptz, numeric, numeric) TO authenticated;
